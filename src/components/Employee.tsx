...
interface Applicant {
  ...
  location: string;
  experience: string;
  gender: string; // Made gender required
  phone: string;
}
...
const Applicants: React.FC = () => {
  ...
  const validateForm = (): boolean => {
    ...
    if (!newApplicant.gender) newErrors.gender = 'Gender is required';
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (editingApplicant) {
        ...
        await fetch('/api/send', {
          ...
          body: JSON.stringify({
            ...
            subject: "Applicant Updated",
            text: `Applicant ${newApplicant.name} has been updated.\nTime: ${new Date().toLocaleString()}\nLocation: Addis Ababa, Ethiopia (approximate)`,
          }),
        });
      } else {
      ...
      let message = editingApplicant ? 'Failed to update applicant' : 'Failed to add applicant';
      if (isAxiosError(error)) {
        message = error.response?.data?.message || message;
            ...
            {filteredApplicants.length === 0 ? (
              ...
              filteredApplicants.map((applicant, index) => (
                ...
                >
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                    {applicant.gender === 'M' ? 'Male' : applicant.gender === 'F' ? 'Female' : 'Unknown'} {applicant.gender} 
                  </td>
